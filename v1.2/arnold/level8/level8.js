...
startNextLevel = function() {
    location.href = "../level9/index.html";
};
...
Player.prototype.checkCollisions = function() {
    ...
    if (choppa.y === player.y) {
        if (choppa.x >= player.x - 30 && choppa.x <= player.x + 30) {
            this.reset();
            startNextLevel();
        }
    }
...
// Update the choppa's position
Choppa.prototype.update = function(dt) { // dt - a time delta between ticks
...
// Reset choppa position after it has gone beyond the screen
Choppa.prototype.reset = function() {
    this.x = 900;
};
...
var allEnemies = [ 
    ...
    new Enemy(700, 220, 'flipped'),
    new Enemy(-2, 300, 'normal'),
    new Enemy(700, 380, 'flipped'),
    new Enemy(-2, 460, 'normal'),
    new Enemy(700, 540, 'flipped')
];
